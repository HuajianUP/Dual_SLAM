...
int ORBmatcher::SearchByGMS(Frame &CurrentFrame, KeyFrame* pKF, vector<cv::Point3f> &mvP3D, vector<cv::Point2f> & mvP2D) //recovery 
{
    ...
    mvP2D.clear();
    mvP3D.clear();
    vector<KeyPoint> kp1 = CurrentFrame.mvKeysUn;
    vector<KeyPoint> kp2 = pKF->mvKeysUn;
    Mat d1 = CurrentFrame.mDescriptors;
    Mat d2 = pKF->mDescriptors;
    ...
    const Size frameSize = CurrentFrame.frameSize;
    ...
    for (size_t i = 0; i < vbInliers.size(); ++i)
    {
        if (vbInliers[i] == true)
        {
            ...
            MapPoint* pMP = vpMapPointsKF[matches_all[i].trainIdx];
            if(!pMP)
                continue;
            if(pMP->isBad())
                continue;
            CurrentFrame.mvpMapPoints[matches_all[i].queryIdx] = pMP;
            cv::Point2f buffP = kp1[matches_all[i].queryIdx].pt;
            mvP2D.push_back(buffP); 
            cv::Mat Pos = pMP->GetWorldPos();  
            mvP3D.push_back(cv::Point3f(Pos.at<double>(0), Pos.at<double>(1), Pos.at<double>(2)));
        }
    }
    return nmatches;
}
...
int ORBmatcher::SearchForInitializationWithGMS(Frame &Frame_1, Frame &Frame_2, vector<int> & vnMatches12) //tracking
{
    ...
    //resize()
    vector<KeyPoint> kp1 = Frame_1.mvKeysUn;
    vector<KeyPoint> kp2 = Frame_2.mvKeysUn;
    Mat d1 = Frame_1.mDescriptors;
    Mat d2 = Frame_2.mDescriptors;
    vector<DMatch> matches_all;//,matches_gms;
    BFMatcher matcher(NORM_HAMMING);
    matcher.match(d1, d2, matches_all);
    //cout<<"all matcher :"<<matches_all.size()<<endl;
    ...
    std::vector<bool> vbInliers;
    const Size frameSize= Frame_1.frameSize;
    gms_matcher gms(kp1,frameSize, kp2,frameSize, matches_all);
    nmatches = gms.GetInlierMask(vbInliers, false, false);
...
int ORBmatcher::SearchByGMS(KeyFrame *pKF1, KeyFrame *pKF2, vector<MapPoint *> &vpMatches12) //loopclosing
{
    const vector<cv::KeyPoint> &kp1 = pKF1->mvKeysUn;
    const vector<MapPoint*> vpMapPoints1 = pKF1->GetMapPointMatches();
    const cv::Mat &d1 = pKF1->mDescriptors;

    const vector<cv::KeyPoint> &kp2 = pKF2->mvKeysUn;
    const vector<MapPoint*> vpMapPoints2 = pKF2->GetMapPointMatches();
    const cv::Mat &d2 = pKF2->mDescriptors;
    ...
    const Size frameSize= pKF1->frameSize;
    int nmatches=0;
    int nmatchesEnd =0;
    gms_matcher gms(kp1,frameSize, kp2,frameSize, matches_all);
    nmatches = gms.GetInlierMask(vbInliers, false, false);
    ...
    for (size_t i = 0; i < vbInliers.size(); ++i)
    {
        ...
        {
            //matches_gms.push_back(matches_all[i]);
            MapPoint* pMP = vpMapPoints2[matches_all[i].trainIdx];
            if(!pMP)
                continue;
            if(pMP->isBad())
                continue;
            vpMatches12[matches_all[i].queryIdx]=pMP;
            nmatchesEnd++;
        }
    }
...
int ORBmatcher::SearchByGMS(KeyFrame *pKF1, KeyFrame *pKF2, vector<pair<size_t, size_t> > &vMatchedPairs) //none
{
    const vector<cv::KeyPoint> &kp1 = pKF1->mvKeysUn;
    //const vector<MapPoint*> vpMapPoints1 = pKF1->GetMapPointMatches();
    const cv::Mat &d1 = pKF1->mDescriptors;
    ...
    for (size_t i = 0; i < vbInliers.size(); ++i)
    {
        if (vbInliers[i] == true)
        {
            MapPoint* pMP1 = pKF1->GetMapPoint(matches_all[i].queryIdx); 
            // If there is already a MapPoint skip
            if(pMP1)
                continue;
            MapPoint* pMP2 = pKF2->GetMapPoint(matches_all[i].trainIdx); 
            // If there is already a MapPoint skip
            if(pMP2)
                continue;
            vMatchedPairs.push_back(make_pair(matches_all[i].queryIdx, matches_all[i].trainIdx));
            nmatchesEnd++;
        }
    }
...
int ORBmatcher::SearchByGMS(Frame &CurrentFrame, const Frame &LastFrame, vector<cv::Point3f> &mvP3D, vector<cv::Point2f> & mvP2D) //tracking 
{
    int nmatches=0;
    int nmatchesEnd=0;
    mvP2D.clear();
    mvP3D.clear();
    vector<KeyPoint> kp1 = CurrentFrame.mvKeysUn;
    vector<KeyPoint> kp2 = LastFrame.mvKeysUn;
    Mat d1 = CurrentFrame.mDescriptors;
    Mat d2 = LastFrame.mDescriptors;
    vector<DMatch> matches_all,matches_gms;
    BFMatcher matcher(NORM_HAMMING);
    matcher.match(d1, d2, matches_all);
    //cout<<"all matcher :"<<matches_all.size()<<endl;
    ...
    for (size_t i = 0; i < vbInliers.size(); ++i)
    {
        ...
        {
            //matches_gms.push_back(matches_all[i]);
            MapPoint* pMP = LastFrame.mvpMapPoints[matches_all[i].trainIdx];
            if(!pMP)
                continue;
            if(pMP->isBad())
                continue;
...
int ORBmatcher::SearchByGMS(Frame &CurrentFrame, const Frame &LastFrame) 
{
    int nmatches=0;
    int nmatchesEnd=0;
    vector<KeyPoint> kp1 = CurrentFrame.mvKeysUn;
