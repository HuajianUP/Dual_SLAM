...
namespace Dual_SLAM
{
	...
	void Recovery::Run()
	{
		...
		UpdateLocalPoints();
		g2o::Sim3 mg2oScw;
		cout<<"test recovery: "<<mpMap->KeyFramesInMap()<<" the number of frames Need Recovery: "<<framesNeedRecovery.size()<<"mvRLocalMapPointsWhenBreak: "<<mvRLocalMapPointsWhenBreak.size()<<endl;
		...
		for(list<Frame>::reverse_iterator it = framesNeedRecovery.rbegin(); it != framesNeedRecovery.rend(); ++it)
        {
            ...
            if(NeedNewKeyFrame())
            {
				...
				{
					cout<<"Trying to RecoverInitialMapPoints ...... "<<endl;
					mState = RECOVERING;
				}
	...
	void Recovery::RecoveryMapByICP(g2o::Sim3 & mg2oScw)
	{
	...
	void Recovery::RecoveryMapBySim3(g2o::Sim3 & mg2oScw)
	{
	    ...
	    mvpCurrentConnectedKFs = mpCurrentKeyFrame->GetVectorCovisibleKeyFrames();
	    mvpCurrentConnectedKFs.push_back(mpCurrentKeyFrame);

	    LoopClosing::KeyFrameAndPose CorrectedSim3, NonCorrectedSim3;
	    CorrectedSim3[mpCurrentKeyFrame]=mg2oScw;
...

   		{
	        ...
	        for(LoopClosing::KeyFrameAndPose::iterator mit=CorrectedSim3.begin(), mend=CorrectedSim3.end(); mit!=mend; mit++)
	        {
	            ...
	            for(size_t iMP=0, endMPi = vpMPsi.size(); iMP<endMPi; iMP++)
	            {
	                ...
	                if(pMPi->isBad())
	                    continue;
	                if(pMPi->mnCorrectedByKF==mpCurrentKeyFrame->mnId)
	                    continue;
	                ...
	                pMPi->mnCorrectedByKF = mpCurrentKeyFrame->mnId;
	        ...
	        for(size_t i=0; i<mvpCurrentMatchedPoints.size(); i++)
	        {
	            if(mvpCurrentMatchedPoints[i])
	            {
	                ...
	                MapPoint* pCurMP = mpCurrentKeyFrame->GetMapPoint(i);
	                if(pCurMP)
	                    pCurMP->Replace(pLoopMP);
	                else
	                {
	                    mpCurrentKeyFrame->AddMapPoint(pLoopMP,i);
	                    pLoopMP->AddObservation(mpCurrentKeyFrame,i);
	    ...
	    Optimizer::OptimizeEssentialGraph(mpMap, mpMatchedKF, mpCurrentKeyFrame, NonCorrectedSim3, CorrectedSim3, LoopConnections, false);
	...
	bool Recovery::DetectInOldMap(g2o::Sim3 & mg2oScw)
	{
		...
		for(vector<KeyFrame*>::iterator itKF = mvRLocalKeyFramesWhenBreak.begin(); itKF != mvRLocalKeyFramesWhenBreak.end(); itKF++)
	    {
    		...
    		//int inliers = 0;
    		int nmatches = matcher.SearchByBoW(mpCurrentKeyFrame,curKF,vpMapPointMatches);
		    ...
		    int nInliers;
		    bool bNoMore;
		    Sim3Solver* pSolver = new Sim3Solver(mpCurrentKeyFrame,curKF,vpMapPointMatches,mbFixScale);
	        ...
	        matcher.SearchBySim3(mpCurrentKeyFrame,curKF,tempMapPoint,s,R,t,7.5);
	        g2o::Sim3 gScm(Converter::toMatrix3d(R),Converter::toVector3d(t),s);
    		nInliers = Optimizer::OptimizeSim3(mpCurrentKeyFrame, curKF, tempMapPoint, gScm, 10, mbFixScale);
			
			if(nInliers < 20)
				continue;

            g2o::Sim3 gSmw(Converter::toMatrix3d(curKF->GetRotation()),Converter::toVector3d(curKF->GetTranslation()),1.0);	       
            ...
            vector<KeyFrame*> vpLoopConnectedKFs = curKF->GetVectorCovisibleKeyFrames();
		    vpLoopConnectedKFs.push_back(curKF);
		    ...
		    for(vector<KeyFrame*>::iterator vit=vpLoopConnectedKFs.begin(); vit!=vpLoopConnectedKFs.end(); vit++)
		    {
		   	...
		   	matcher.SearchByProjection(mpCurrentKeyFrame, mScw, mvRLocalMapPointsWhenBreak, mvpCurrentMatchedPoints,10); //mvpLoopMapPoints
	...
	/*
	bool Recovery::ComputeSim3(KeyFrame* mpCurrentKF, KeyFrame* MatchedKF,vector<MapPoint*> vpMapPointMatches)
	{
	    ...
	    int nInliers;
	    bool bNoMore;
	    Sim3Solver* pSolver = new Sim3Solver(mpCurrentKF,MatchedKF,vpMapPointMatches,mbFixScale);
	    pSolver->SetRansacParameters(0.99,20,300);
	    cv::Mat Scm  = pSolver->iterate(5,bNoMore,vbInliers,nInliers);
	    cout<<"start ComputeSim3 "<<nInliers<<endl;
	    ...
	    if(!Scm.empty() && nInliers > 20)
	    {
            ...
            for(size_t j=0; j<vbInliers.size(); j++)
            {
                if(vbInliers[j])
                {
                	//vpMapPointMatches[j] = static_cast<MapPoint*>(NULL);
                	//if(vpMapPointMatches[j] && !vpMapPointMatches[j]->isBad())
                  	tempMapPoint[j]=vpMapPointMatches[j];
                }
            }
            //mvpCurrentMatchedPoints = temp;
	        //cout<<"vbInliers.size()"<<vbInliers.size()<<endl;
	        cv::Mat R = pSolver->GetEstimatedRotation();
	        cv::Mat t = pSolver->GetEstimatedTranslation();
	        const float s = pSolver->GetEstimatedScale();

	        matcher.SearchBySim3(mpCurrentKF,MatchedKF,tempMapPoint,s,R,t,7.5);

	        //cout<<"Optimizer::OptimizeSim3************"<<endl;
	        g2o::Sim3 gScm(Converter::toMatrix3d(R),Converter::toVector3d(t),s);

	        const int nInliers = Optimizer::OptimizeSim3(mpCurrentKF, MatchedKF, tempMapPoint, gScm, 10, mbFixScale);
	        ...
	        if(nInliers >= 20)
	        {
	            ...
	            g2o::Sim3  tempSim3 = gScm*gSmw; //g2o::Sim3 
	            //mg2oScw = tempSim3;
	            cout<<"mg2oScw = gScm*gSmw;"<<endl;
	            //return false;
	            mScw = Converter::toCvMat(mg2oScw);
	            //cout<<"mScw = Converter::toCvMat(mg2oScw);"<<endl;
	            mvpCurrentMatchedPoints = tempMapPoint;
	        }
			//return false;
	    }
	...
	*/
	...
	void Recovery::SearchLocalPoints()
	{
	    ...
	    if(nToMatch>0)
	    {
	        ...
	        int th = 1;
	        matcher.SearchByProjection(mCurrentFrame,mvRLocalMapPoints,th);
	    }
	}
